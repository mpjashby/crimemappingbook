...
Now we know the correct column name, we can extract it using the correct name:
...
```{r finding-exercise1, error=TRUE}
library(tidyverse)
...
```{r finding-exercise2, error=TRUE}
frauds |> 
...
```{r finding-exercise4, error=TRUE}
library(sf)
...
```{r reprex-exercise1-setup, echo=FALSE, include=FALSE}
...
So we can change the code below so that it uses the `mpg` dataset rather than 
loads data from the `road_deaths_data.csv` file:
