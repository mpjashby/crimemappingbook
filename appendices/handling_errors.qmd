...
2. Make sure your code **follows the good practices summarised in @sec-preventing-bugs**, which make problems -- especially those caused by typos -- easier to see. Save the script so that Air formats it, and keep each function in a pipeline on a separate line so that you can comment out individual steps.
