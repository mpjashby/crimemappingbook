...
```r
crimemapping::tutorial("06_map_context")
```
...
using the `subtitle` argument to the `labs()` function. 


::: {.tutorial}

Use the code above as a template to add a subtitle to your map explaining that 
the map shows fatal and non-fatal shootings in 2019. 
...
that order -- to remember the order, think '**tr**ou**bl**e'. The following code 
changes the margin around the map subtitle. 


::: {.tutorial}

Change this code so that it *also* makes the subtitle 80% of the default size 
*and* changes the caption so that it is left aligned.
