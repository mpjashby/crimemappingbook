...
not all of the rows relate to bicycle thefts. The `type` column shows that the
dataset also includes thefts from vehicles, for example. To choose only those 
rows containing bicycle thefts, we can use the `filter()` function from the
`dplyr` package.
...
We can change the black circles on the map above into red squares using the
`colour` and `shape` arguments to the `geom_sf()` function.
...
points slightly smaller at the same time. You can use the `alpha` and `size` 
...
zoom_map <- map(9:14, function(zoom_level) {
  ggplot() + 
...
basemaps_map <- c(
  "osm", "opencycle", "hotstyle",  "stamenbw", "osmtransport", 
  "thunderforestlandscape", "thunderforestoutdoors", "cartodark", "cartolight"
) |> 
  map(function (x) {
    ggplot() +
      annotation_map_tile(type = x, zoom = 13) +
      geom_sf(data = jdi, colour = NA, fill = NA) +
