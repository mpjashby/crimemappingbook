...
**Walk through each stage in the process of creating a map showing the density of crime.**
...
```r
crimemapping::tutorial("05_your_second_crime_map")
```
...
![](images/vancouver_bike_thefts.jpg){width=80%}
...
{{< video https://youtu.be/hbOGp51nYGI >}}
...
{{< video https://youtu.be/inJk4BEUm7A >}}
...
{{< video https://youtu.be/G5vxLucllr4 >}}
...
{{< video https://youtu.be/A6HH-zDl9a0 >}}
...
Thinking back to the tutorial on data wrangling, we can use the `read_csv()`
function from the `readr` package to load this data into a tibble called 
`thefts`
...
Now that you have stored the data in the `thefts` object, we can use the 
`head()` function to view the first few rows of data.
...
![](images/utm_vancouver.jpg){width=80%}
...
Thinking back to the chapter on data wrangling, we can use the `filter()`
function to choose only the rows of data that relate to bicycle
thefts and store it in a new object called `bike_thefts`
...
We can change the code above so that the points on our map are red squares 
instead of black circles (`red` is one of the 657 R colour names).
...
![](images/kde_explanation.jpg){width=80%}
...
![](images/bandwidth.jpg){width=100%}
...
detail in your map by using a smaller bandwidth, you could use `adjust = 0.5` 
or `adjust = 1/2` to set the bandwidth to be half of the default bandwidth.
...
```{r include=FALSE}
# SUPPRESS THE PROGRESS BAR OUTPUT FROM `hotspot_kde()`
hotspot_kde_silent_1 <- hotspot_kde(bike_thefts, bandwidth_adjust = 1/2) |> 
  ggplot() +
  geom_sf(aes(fill = kde), colour = NA)
```

```{r}
hotspot_kde_silent_1
```
...
```{r include=FALSE}
# SUPPRESS PROGRESS BAR PRODUCED BY `hotspot_kde()`
