...
![](../images/chapter_05_second_crime_map_illustration.png){fig-alt=""}
...
::: {.callout-quiz .callout title="Naming files"}
...
file_names_quiz <- c(
  ...
  "`data2.csv`",
  answer = "`vancouver_thefts_2020.csv`",
  "`vancouver-thefts-2020 FINAL.csv`"
)
```

`r webexercises::longmcq(file_names_quiz)`
...
## Spatial data in R {#sec-spatial-data}
...
To get started, create a new R script file to hold the permanent code for this analysis. Give the file the name `chapter_05.R` and save it in the same directory as the R script files we created in previous chapters. You can remind yourself how to do that by looking back to @sec-permanent-code.
...
Thinking back to what we learned in @sec-load-data, add code to `chapter_05.R` that downloads the dataset to `data/raw/vancouver_thefts.csv.gz`, then loads the local file and stores the data in an object called `thefts`. If you need help, you can click the 'Solution' button below.
...
Typos are one of the most frequent causes of errors in any coding language. As we learned in @sec-naming-objects, column names should use *snake case*: lower-case letters with words separated by underscores (`_`). This makes code easier to read and means you do not have to remember whether a column was called `crime_count`, `crimecount`, `CrimeCount` or `CRIMECOUNT`.
...
utm_zones <- st_read(
  "https://opendata.arcgis.com/datasets/b294795270aa4fb3bd25286bf09edc51_0.geojson"
) |>
  janitor::clean_names() |>
  mutate(
...
utm_centroids <- utm_zones |>
  ...
  mutate(
    zone_label = str_glue(
      "zone {zone}{str_to_upper(str_sub(hemisphere, 1, 1))}"
    )
  ) |>
  st_crop(bbox)
...
utm_vancouver <- ggplot() +
  ...
  geom_sf(
    data = st_crop(
      filter(utm_zones, zone == 10, hemisphere == "northern"),
      bbox
    ),
    colour = "black",
    fill = NA
...
## Organising code with comments {#sec-comments}
...
::: {.callout-quiz .callout title="Comments"}
...
Save `chapter_05.R` by pressing {{< kbd win="Ctrl-S" mac="Command-S" >}}. Then start a new R session by clicking the **Restart R** (**&#10227;**) button in Positron's **Console** panel. This creates a blank canvas for the next chapter.
